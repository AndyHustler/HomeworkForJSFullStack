...
class TarifSearch {
    ...
    addTarif(city1: string, city2: string, val: number): void {
    ...
    delCity(value: string): boolean | undefined {
        if (this.citys[value]) {
    ...
    changeCity(oldKey: string, newKey: string): boolean | undefined{
        if (this.citys[oldKey] && !this.citys[newKey]) {
            let ch = Object.keys(this.citys[oldKey]);
            ch.forEach(k =>{
                let x: number[] = Object.values(this.citys[k]);
                let y: string[] = Object.keys(this.citys[k]);
    ...
    minTarif(cityOut: string, cityIn: string) {
        if (!(cityOut in this.citys) || !(cityIn in this.citys)) throw new Error('Таких городов нет в списке. Список городов: ' + Object.keys(this.citys));
        let visited = {};
        let distances = {};
        let previous = {};
        ...
        function handleVertex(vertex: string, obj: object) {
            let activeVertexDistance: number = distances[vertex]; 
        ...
        function findNearestVertex(distances: object, visited: object) {
            let minDistance = Infinity;
            let nearestVertex: string | null = null;
          
            Object.keys(distances).forEach((vertex: string) => {
        ...
        let activeVertex: any = findNearestVertex(distances, visited);
